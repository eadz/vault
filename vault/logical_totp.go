...
// TotpBackendFactory returns a TotpBackend
// with leases switched off
func TotpBackendFactory(conf *logical.BackendConfig) (logical.Backend, error) {
	return LeaseSwitchedTotpBackend(conf, false)
}

// TotpBackendWithLeasesFactory returns a TotpBackend
// with leases switched on
func LeasedTotpBackendFactory(conf *logical.BackendConfig) (logical.Backend, error) {
	return LeaseSwitchedTotpBackend(conf, true)
}

// LeaseSwitchedTotpBackendFactory returns a TotpBackend
// with leases switched on or off
func LeaseSwitchedTotpBackend(conf *logical.BackendConfig, leases bool) (logical.Backend, error) {
	var b TotpBackend
	b.generateLeases = leases
	b.Backend = &framework.Backend{
		Help: strings.TrimSpace(TotpHelp),
		...
		Paths: []*framework.Path{
			&framework.Path{
				...
				HelpSynopsis:    strings.TrimSpace(TotpHelpSynopsis),
				HelpDescription: strings.TrimSpace(TotpHelpDescription),
			},
		},
	}
...
// TotpBackend is used storing secrets directly into the physical
...
func (b *TotpBackend) handleRevoke(
	req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	// This is a no-op
	return nil, nil
}

func (b *TotpBackend) handleExistenceCheck(
...
func (b *TotpBackend) handleRead(
	...
	totpSecret, _ = rawData["totpSecret"].(string)
	if len(totpSecret) > 0 {
		k, _ := base32.StdEncoding.DecodeString(totpSecret)
		totp, _ := Totp(k, time.Now().Unix(), sha1.New, 6)
		resp.Data["totpSecret"] = totp
	} else {
...
func (b *TotpBackend) handleWrite(
	req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
...
func (b *TotpBackend) handleDelete(
	req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
...
func (b *TotpBackend) GeneratesLeases() bool {
